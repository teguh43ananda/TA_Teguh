...
# from common.gait_features import GaitFeatureExtractor
...
#         # sessions/<stamp>/raw_points/
#         # sessions/<stamp>/gait_features/
#         #========================================
#         # self.base_dir   = os.path.join(os.getcwd(), "dataset", "teguh") # UBAH INI UNTUK PENGAMBILAN DATA =============================================================
#         # self.points_dir = os.path.join(self.base_dir, "point_cloud")
#         # self.gait_dir   = os.path.join(self.base_dir, "gait_features")
#         # os.makedirs(self.points_dir, exist_ok=True)
#         # os.makedirs(self.gait_dir,   exist_ok=True)

#         # # Lokasi CSV raw points (7 kolom: x,y,z,doppler,SNR + meta)
#         # self.points_csv_path = os.path.join(self.points_dir, "dataset_points.csv")

#         # # Inisialisasi ekstraktor gait
#         # self.gait = GaitFeatureExtractor(
#         #     out_dir=self.gait_dir,
#         #     axis_forward='x',     # ubah 'y' jika arah maju sumbu Y
#         #     use_signal='z',       # 'z' (minima cz) atau 'v' (peak v_forward)
#         #     min_step_interval=0.28,
#         #     smooth_alpha=0.2
#         # )

#         # ====== SIMPLE STRUCTURE + AUTO NUMBER Jalan{N} ======
#         # <SUBJEK> kamu ubah manual di sini:
#         self.base_dir = os.path.join(os.getcwd(), "dataset", "udin")  # ganti "teguh" sesuai subjek

#         # Buat dua folder datar: raw/ dan gait/
#         raw_dir  = os.path.join(self.base_dir, "raw")
#         gait_dir = os.path.join(self.base_dir, "gait")
#         os.makedirs(raw_dir,  exist_ok=True)
#         os.makedirs(gait_dir, exist_ok=True)

#         # Cari nomor berikutnya: raw/Jalan{N}.csv
#         import glob, re
#         existing = glob.glob(os.path.join(raw_dir, "Jalan*.csv"))
#         nums = []
#         for p in existing:
#             m = re.search(r"Jalan(\d+)\.csv$", os.path.basename(p))
#             if m:
#                 nums.append(int(m.group(1)))
#         next_n = (max(nums) + 1) if nums else 1
#         self.motion_id = f"Jalan{next_n}"

#         # Path file tujuan (tanpa subfolder sesi)
#         self.points_csv_path = os.path.join(raw_dir,  f"{self.motion_id}.csv")

#         # Inisialisasi ekstraktor gait → tulis ke dataset/<SUBJEK>/gait/Jalan{N}.csv
#         self.gait = GaitFeatureExtractor(
#             out_dir=gait_dir,
#             axis_forward='x',     # ubah 'y' jika arah maju sumbu Y
#             use_signal='z',       # 'z' (minima cz) atau 'v' (peak v_forward)
#             min_step_interval=0.28,
#             smooth_alpha=0.2,
#             csv_name=f"{self.motion_id}.csv"
#         )
#         # ====== END SIMPLE STRUCTURE ======
...
#                 # (Opsional) mekanisme replay JSON lamamu — biarkan jika memang dipakai
#                 if getattr(self.parser, "saveBinary", 0) == 1:
#                     frameJSON = {'frameData': outputDict, 'timestamp': time.time() * 1000}
#                     # buffer replay (jika parser.frames tersedia)
#                     if hasattr(self.parser, "frames"):
#                         self.parser.frames.append(frameJSON)

#                 # --- SIMPAN RAW POINTS (CSV minimal 7 kolom) ---
#                 try:
#                     self._saveDataToCsvPoints(outputDict, self.points_csv_path)
#                 except Exception as e:
#                     log.error(f"Save raw points error: {e}")

#                 # --- UPDATE GAIT FEATURES PER FRAME ---
#                 try:
#                     self.gait.update(outputDict)
#                 except Exception as e:
#                     log.error(f"Gait extractor error: {e}")

#         except Exception as e:
#             log.error(f"parseUartThread.run() error: {e}")

#     def _saveDataToCsvPoints(self, outputDict, csv_path):
#         """
#         Tulis raw point cloud minimal per titik:
#         columns: timestamp, frame, x, y, z, doppler, SNR
#         """
...
#         # timestamp
#         ts = outputDict.get('timeStamp')
#         if ts is None and outputDict.get('unixTimeMs') is not None:
...
#             w = csv.writer(f)
#             if write_header:
#                 w.writerow(["timestamp","frame","x","y","z","doppler","SNR"])

#             # Asumsi: pc[i][:5] = x, y, z, doppler, SNR
#             for p in pc:
#                 x, y, z, doppler, SNR = p[:5]
#                 w.writerow([
#                     f"{float(ts):.6f}",
#                     frame,
#                     f"{float(x):.6f}",
